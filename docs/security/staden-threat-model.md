...
**Scopeankare:** Git `00cd68b` plus den ännu ej hostat skannade produktkandidaten i arbetskatalogen, analyserat 2026-08-23
...
- Supabase är manuellt länkat som framtida system of record. Nuvarande klient använder endast projektets publika URL/publishable key för en hälsokontroll. Service role får aldrig exponeras som `NEXT_PUBLIC_*`.
...
- **Supabase-klient:** `src/lib/supabase/client.ts` binder URL:en till det avsedda projektet, använder en publishable key, stänger av sessionslagring/refresh och anropar endast Auth health med åtta sekunders timeout.
...
- **Future Auth/data/ingestion:** antagna komponenter; migrationer, tabeller, RLS-policyer, serverjobb och CMS saknas och får inte betraktas som implementerade kontroller.
...
- **Browser → Supabase Auth health:** fast HTTPS-origin, publishable key i `apikey`-header, ingen session och ingen datafråga. Ett HTTP 200 bevisar reachability/key acceptance men inte Data API, grants, RLS eller Auth-policy.
...
- **Future data-enabled client → Supabase:** konton, centrala listor och platsdata är antagna. AuthN, RLS, schema validation, origin controls och abuse limits är okända eftersom flöden och migrationer saknas.
...
flowchart LR
    ...
    App -->|"publishable key; health only"| Cloud["Hosted Supabase"]
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| TM-001 | Lågprivilegierad användare | Användartabeller skapas; RLS/ownership är fel eller saknas | Manipulerar direkta Supabase-anrop för BOLA/IDOR | Integritetsintrång och manipulation av privata listor/profiler | User data, RLS, editorial integrity | Nuvarande klient har endast health, ingen Auth/session/datafråga; sessionpersistens är avstängd | Hosted grants/RLS och negativa tester saknas | Revoke-by-default; RLS på varje exponerad tabell; tvåanvändartester; separata admin-RPC:er | Nekade policyförsök, massläsning per principal | Ej realiserad nu; medel när dataklienten införs | Hög | Hög villkorad |
