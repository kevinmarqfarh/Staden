...
**Basrevision:** `bef8cd8` på `main`; säkerhetshärdningen infördes i `7d2baba`, scannerfixen verifierades i `8858b4f` och senaste hosted baseline dokumenterades i `861a561`
...
| Applikationskvalitet | `npm run lint` och `npm run build` med låst dependency tree | Båda passerade; Next.js 16.3.2 producerade statiska routes för `/`, `/_not-found` och `/icon.svg` | Lokal evidens. Hostad Vercel-build verifieras separat efter push. |
...
| Hosted Security CI | GitHub Actions [run 32608888382](https://github.com/kevinmarqfarh/Staden/actions/runs/32608888382) på commit `861a561` | **Success:** applicability, TruffleHog, Zizmor och strict required gate passerade | Den körningen föregår appmanifestet; CodeQL och SCA för apprevisionen måste verifieras i en ny hosted run. |
...
- Vercel-projektets inställningar, root directory, domän, environment variables, deployment-ID och hostad respons. Lokal appkod bevisar inte aktuell deployment.
