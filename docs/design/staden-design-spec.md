...
3. **KVARTERSPLANEN** — an eight-tile grid, 2×4 on mobile, each tile 165×88px at 390px (350 − 20 gap ÷ 2 = 165). Göteborg's eight: `CENTRUM · NORDSTAN · HAGA · LINNÉ · MAJORNA · HISINGEN · GAMLESTADEN · ÖRGRYTE`. Selecting one recomputes every `gangminuter` from that kvarter's centroid node. **Zero geolocation permission is ever requested.** Walk minutes and terrain words are real, from OSM pedestrian routing computed at ingest into a `gangtider(fran_kvarter, objekt_id, minuter, terrangord)` table. Privacy-averse and older users get the whole product, not a hollowed-out one. (If `geolocation=(self)` is later enabled in `next.config.ts` — it is currently `geolocation=()`, which vetoes the feature outright — a "Använd min plats" row appears *inside* KVARTERSPLANEN as a ninth option, never as a prompt on load.)
...
**KVARTERSVÄLJARE** — an 8-tile neighbourhood grid (Centrum, Vasastan, Linné, Majorna, Haga, Örgryte, Hisingen, Norr om älven), each tile 2 columns wide × 72px tall. Selecting one produces real walk distances **with zero location permission ever requested** — `next.config.ts` keeps `geolocation=()` and the product never asks. Distances are computed from the kvarter centroid and the object's stored OSM coordinate.
...
- `next.config.ts` keeps `Permissions-Policy: camera=(), microphone=(), geolocation=()` **unchanged**. Distance and "near me" are served entirely by the kvarter picker from stored OSM coordinates, so the product never requests a location permission, never shows a permission prompt, and has no denied-permission state to design. This is a feature for privacy-averse and older users, not a compromise.
