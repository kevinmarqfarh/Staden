    ...
    >
      ...
      >
        ...
        <section className="saved-pocket-lists" aria-labelledby="list-title">
          ...
          <form className="saved-pocket-create" onSubmit={createList}>
            <label htmlFor="saved-pocket-list-name">Ny lista</label>
            <div>
              <input
          ...
          {selectedListId && editingListId === selectedListId ? (
            <form className="saved-pocket-rename" onSubmit={renameSelectedList}>
              ...
              <div>
                ...
                />
                <button type="submit" aria-label="Spara nytt listnamn"><Check size={17} /></button>
                <button
                  type="button"
        ...
        <section className="saved-pocket-content" aria-labelledby="saved-items-title">
          <div className="saved-pocket-section-heading">
            ...
            <div>
              <h3 id="saved-items-title">
                {selectedListId === null
                  ? "Allt sparat"
                  : pocketState.lists.find((list) => list.id === selectedListId)
                      ?.name ?? "Sparat"}
              </h3>
              <p aria-live="polite">{visibleItems.length} i det här urvalet</p>
            </div>
          </div>
          ...
          {visibleItems.length ? (
            <ul className="saved-pocket-items">
              {visibleItems.map((savedItem) => {
                ...
                const metadata = isCulture
                  ? `${savedItem.item.dateLabel} · ${savedItem.item.venue}`
                  : `${savedItem.item.cuisine} · ${savedItem.item.area}`;
            ...
            <div className="saved-pocket-empty">
              ...
              <p>
                {savedItems.length
                  ? "Välj en lista på ett sparat kort för att lägga det här."
